...
FIXES for 'Stream idle timeout':
  1. Parallel data download with ThreadPoolExecutor
  2. Vectorised regime series (weekly step, not per-day loop)
  3. flush=True heartbeats so the stream never goes idle
  4. --quick and --no-hmm flags for fast smoke-tests
"""
import warnings
...
# -- FIX 1: Parallel data loader ---------------------------------------------
def _download_one(symbol: str, years: int) -> tuple:
    cache_path = config.DATA_DIR / f"{symbol}_bt_{years}y.parquet"
    if cache_path.exists():
        df = pd.read_parquet(cache_path)
        print(f"  cache {symbol}: {len(df)} bars", flush=True)
        return symbol, df
    print(f"  downloading {symbol} {years}yr ...", flush=True)
    try:
        raw = yf.download(
            f"{symbol}.NS",
            period=f"{years}y",
        ...
        print(f"  saved {symbol}: {len(df)} bars", flush=True)
        return symbol, df
...
def load_all_data(symbols: list, years: int = BACKTEST_YEARS) -> dict:
    """FIX 1 -- download all symbols in parallel (max 6 threads)."""
    all_data = {}
...
class BacktestEngine:
    ...
    def run(self, all_data: dict) -> dict:
        ...
        print(f"  {len(sorted_dates)} trading days x {len(all_data)} symbols", flush=True)
        ...
        for i, (sym, df) in enumerate(all_data.items(), 1):
            ...
            try:
                ...
                if i % 5 == 0:
                    print(f"    signals {i}/{len(all_data)}", flush=True)
            except Exception as exc:
                logger.warning(f"Signal compute failed for {sym}: {exc}")

        print("  Computing regime series...", flush=True)
        ...
        for idx, dt in enumerate(sorted_dates):
            # FIX 3 -- heartbeat every 100 days keeps the stream alive
            if idx % 100 == 0:
                print(f"    day {idx}/{total_days} ...", flush=True)
    ...
    def _compute_regime_series_fast(self, all_data: dict) -> dict:
        """FIX 2 -- predict once per 5 days then forward-fill (5x fewer HMM calls)."""
        ...
        if not self._hmm.is_trained():
            try:
                print("  Training HMM...", flush=True)
                self._hmm.train(nifty)
                print("  HMM trained.", flush=True)
            except Exception as exc:
        ...
        for i in range(window, len(nifty), step):
            ...
            try:
                last_regime = self._hmm.predict(chunk)
            except Exception:
                pass
    ...
    def _compile_results(self) -> dict:
        ...
        if len(df_d) > 0:
            ...
            max_dd      = round(float((equity - running_max).min()), 2)
        else:
            max_dd = 0.0
...
def print_report(r: dict, label: str = "WITH HMM") -> None:
    ...
    print(f" {BACKTEST_YEARS}-year | {len(SYMBOLS)} symbols | Rs{CAPITAL:,.0f} capital")
    print(bar)
    if "error" in r:
    ...
    print()
    print(f" GROSS P&L total:  Rs{r['gross_total']:>10,.2f}")
    print(f" Total charges:    Rs{r['charges_total']:>10,.2f}")
    print(f" NET P&L total:    Rs{r['net_total']:>10,.2f}")
    print()
    print(f" Avg GROSS/day:    Rs{r['avg_gross_day']:>8,.2f}")
    print(f" Avg charges/day:  Rs{r['avg_charges_day']:>8,.2f}")
    print(f" Avg NET/day:      Rs{r['avg_net_day']:>8,.2f}")
    print()
    print(f" Sharpe ratio:     {r['sharpe']:>6.2f}")
    print(f" Max drawdown:     Rs{r['max_drawdown']:>8,.2f}")
    print(f" Target hit rate:  {r['target_rate']:>5.1f}% (days >= Rs{TARGET_GROSS:,.0f} gross)")
    print(bar)
    ...
    if r.get("monthly") is not None:
        ...
        for period, row in r["monthly"].tail(12).iterrows():
            print(
                f"  {period}: "
                f"Gross=Rs{row['gross_pnl']:>8,.0f}  "
                f"Charges=Rs{row['charges']:>6,.0f}  "
                f"Net=Rs{row['net_pnl']:>8,.0f}  "
                f"Trades={int(row['num_trades'])}"
            )
    print()
    if r["avg_net_day"] >= TARGET_GROSS - 500:
        print(f" System meets Rs{TARGET_GROSS-500:,.0f}/day net target")
    else:
        print(f" Net/day Rs{r['avg_net_day']:,.0f} below Rs{TARGET_GROSS-500:,.0f} target")
    print()
...
def main() -> None:
    ...
    parser.add_argument("--quick",  action="store_true",
                        help="Use 1-year data only (fast smoke test)")
    ...
    print("\n" + "=" * 52)
    print(f" UPSTOX {years}-YEAR BACKTEST")
    print(f" Symbols : {', '.join(SYMBOLS)}")
    print(f" Capital : Rs{CAPITAL:,.0f}")
    print(f" Charges : Rs{CHARGES_PER:.0f}/trade (round-trip)")
    print("=" * 52)

    print("\nStep 1: Loading historical data (parallel)...")
